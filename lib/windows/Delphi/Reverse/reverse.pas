...
begin
  ...
  chan := BASS_StreamCreateFile(FALSE, pChar(OpenDialog1.filename), 0, 0, BASS_STREAM_DECODE Or BASS_STREAM_PRESCAN {$IFDEF UNICODE} or BASS_UNICODE {$ENDIF});
  If (chan = 0) Then chan := BASS_MusicLoad(FALSE, pChar(OpenDialog1.filename), 0, 0, BASS_MUSIC_DECODE Or BASS_MUSIC_RAMP Or BASS_MUSIC_PRESCAN {$IFDEF UNICODE} or BASS_UNICODE {$ENDIF}, 0);
