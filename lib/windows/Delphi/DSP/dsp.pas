...
begin
  ...
  // check for a stream
  chan := BASS_StreamCreateFile(FALSE, pChar(od.filename), 0, 0, BASS_SAMPLE_LOOP Or BASS_SAMPLE_FLOAT {$IFDEF UNICODE} or BASS_UNICODE {$ENDIF});

  // check for MOD
  If chan = 0 Then chan := BASS_MusicLoad(FALSE, pChar(od.filename), 0, 0, BASS_MUSIC_LOOP Or BASS_MUSIC_RAMP Or BASS_SAMPLE_FLOAT {$IFDEF UNICODE} or BASS_UNICODE {$ENDIF}, 0);
