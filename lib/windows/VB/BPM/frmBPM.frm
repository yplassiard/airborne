...
Begin VB.Form frmBPM 
   ...
   Begin VB.Frame frameBPM 
      ...
      Begin VB.Label lblProcess 
         Alignment       =   2  'Center
         Caption         =   "Detection progress of a decoding BPM:"
         Height          =   195
         Left            =   240
         TabIndex        =   14
         Top             =   3240
...
'============================================================================
' frmBPM.frm - Copyright (c) 2003-2013 (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                                    [http://www.jobnik.org]
'                                                    [bass_fx @ jobnik .org]
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If (uTimerID) Then Call timeKillEvent(uTimerID) ' free multimedia timer
End Sub
...
Private Sub cmdOpenFP_Click()
    ...
    With sldTempo(1)
        ...
        .max = info.freq * 1.3          ' by +30%
        .min = info.freq * 0.7          ' by -30%
    ...
    Call DecodingBPM(True, sldPosition.value, IIf((sldPosition.value + Val(txtPeriod.Text)) >= sldPosition.max, sldPosition.max - 1, sldPosition.value + Val(txtPeriod.Text)))
End Sub
...
Private Sub sldPosition_Change()
    ...
    Call DecodingBPM(False, sldPosition.value, IIf((sldPosition.value + Val(txtPeriod.Text)) >= sldPosition.max, sldPosition.max - 1, sldPosition.value + Val(txtPeriod.Text)))
End Sub
...
Private Sub sldTempo_Scroll(index As Integer)
    ...
    ' update the bpm view
    lblBPM.Caption = "BPM: " & Format(GetNewBPM(bpmValue), "0.00")
