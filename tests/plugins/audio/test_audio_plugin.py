...
class TestAudioPluginInitialization:
    ...
    def test_initialize(self, context: PluginContext) -> None:
        ...
        # Should subscribe to nine message topics: POSITION_UPDATED, TTS_SPEAK, TTS_INTERRUPT, PROXIMITY_BEEP, CONTROL_INPUT, ENGINE_STATE, SYSTEM_STATE, electrical.master_switch, audio.play_click
        assert context.message_queue.subscribe.call_count == 9
...
class TestAudioPluginShutdown:
    ...
    def test_shutdown(self, plugin: AudioPlugin) -> None:
        ...
        # Should unsubscribe from messages (POSITION_UPDATED, TTS_SPEAK, CONTROL_INPUT, TTS_INTERRUPT, ENGINE_STATE, SYSTEM_STATE, PROXIMITY_BEEP, electrical.master_switch, audio.play_click)
        assert plugin.context.message_queue.unsubscribe.call_count == 9
